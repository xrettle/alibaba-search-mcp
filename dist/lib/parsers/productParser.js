...
exports.parseProductPage = parseProductPage;
...
async function parseProductPage(page) {
    ...
    const extracted = await page.evaluate(() => {
        ...
        // 2. Price Tiers
        const priceTiers = [];
        const tierEls = document.querySelectorAll(".price-tier, .price-range, .price-item, .price-ranges-list > div, .promotion-price-item, tr.price-row");
        tierEls.forEach((el) => {
        ...
        // 3. MOQ
        const moqEl = document.querySelector(".moq-number, .moq-value, .product-moq, .min-order-value, [data-content='moq'], .moq-val");
        const MOQ = moqEl ? moqEl.textContent : null;
        // 4. Specs
        const specs = {};
        const attributeItems = Array.from(document.querySelectorAll(".do-entry-item, .attribute-item, .product-property-item, .spec-item, tr.attribute-row, .product-attribute tr"));
        ...
        // 5. Lead Time
        const leadTimeEl = document.querySelector(".lead-time-wrapper, .lead-time-list, .shipping-lead-time, .delivery-info, .lead-time-table");
        const leadTime = leadTimeEl ? leadTimeEl.textContent : null;
        // 6. Images
        const images = [];
        const imgEls = document.querySelectorAll(".main-image, .detail-image, .detail-gallery img, .slider-item img, img.image-nav-item, .pic-list img");
        imgEls.forEach((img) => {
        ...
        // 7. Description text
        const descEl = document.querySelector("#product-detail, .product-detail-introduction, .description-detail, .rich-text-description, .product-description, #detail-description");
        const description = descEl ? descEl.textContent : null;
        // 8. Supplier Summary
        const supplierEl = document.querySelector(".supplier-card, .supplier-info, .company-profile-card, .store-card, .company-info-panel");
        const supplierSummary = supplierEl ? supplierEl.textContent : null;
        // 9. Storefront/Company links
        const storefrontLinks = [];
        const links = Array.from(document.querySelectorAll("a[href*='.en.alibaba.com']"));
        links.forEach((l) => {
    ...
    const data = {
        title: (0, utils_1.cleanText)(extracted.title),
        description: (0, utils_1.cleanText)(extracted.description),
        priceTiers: extracted.priceTiers,
        MOQ: (0, utils_1.cleanText)(extracted.MOQ),
        specs: extracted.specs,
        leadTime: (0, utils_1.cleanText)(extracted.leadTime),
        images: extracted.images.length > 0 ? extracted.images : null,
        supplierSummary: (0, utils_1.cleanText)(extracted.supplierSummary),
